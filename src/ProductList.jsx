...
function ProductList({ onHomeClick }) {
    // Note on the visibility logic.
    // App.jsx defines the parent component (App) that passes down the onHomeClick function as a prop to ProductList. 
    // The onHomeClick function is responsible for setting the showProductList state in App.jsx to false, which hides the ProductList 
    // component and shows the landing page instead. In ProductList.jsx, we define two local state variables: showCart and showPlants
    // to control the visibility of the CartItem component. When the user clicks on the cart icon, we set showCart to true, which
    // renders the CartItem component. When the user clicks "Continue Shopping" in the CartItem component, we call the 
    // handleContinueShopping function, which sets showCart back to false, hiding the CartItem and showing the product grid again.
    //
    // So we have the return value of the ProductList component returning the jsx for the product grid and the CartItem component (in CartItem.jsx)
    // returning the cart grid. They are always there and updated by Redux to reflect state changes, but we use conditional rendering to control 
    // which one is visible based on the state variables.
    // Put this another way (as I'm about to write this code), neither ProductList nor CartItem are being mounted or unmounted. 
    // They are both always mounted, and neither know nor care whether they are visible or not. That is the App component's concern.
    // The jsx returned by ProductList will always return an identical prduct grid, but I will need to conditionally render the 'add to cart'
    // buttons for each product, and the number inside the cart icon.
